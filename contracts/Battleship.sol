...
contract BattleShipGame is SigVerifier {
    ...
    uint public constant NO_SHIP_PIECES = 14;
    ...
    // player History
    mapping(address => HittedCoordinate[]) public playerHistory;
    ...
    event LastTurnShot(uint turn);
    event LastTurnReport(uint turn);
    event GameOver();
    event GameStarted();
    ...
    function reportHits(ShipShotProof[] memory _shotSignatures) public {
        ...
        for (uint i = 0; i < _shotSignatures.length; i++) {
            ...
            playerHistory[msg.sender].push(HittedCoordinate({x: coord.x, y: coord.y, isHit: _isHit}));
        }
        playerHasReportedHits[msg.sender] = true;
        if (hasReportedShots() == true){
            emit LastTurnReport(turn);
        }
    }
    ...
    function getPlayerHistory(address _player) public view returns (HittedCoordinate[] memory) {
